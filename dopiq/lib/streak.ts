 ...
 * - If the user hasn't logged in before, starts the streak at 1.
 * - If the last login was yesterday, increments by 1.
 * - If it was today, leaves the value alone (no-op write avoided).
 * - If it was 2+ days ago, resets to 1.
 * Returns the streak value AFTER any update.
 */
export async function recordLogin(
  ...
  if (!last) {
    ...
    if (diffDays <= 0) {
      // Same UTC day — don't touch the count
